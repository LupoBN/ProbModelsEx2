...
class EM(object):
    ...
    def _calculate_wti_numerator(self, i):
        # TODO: Add the smoothing.
        wti = self._alphas[i]
        for word in self._P[i]:
            wti *= math.pow(self._P[i][word], self._ntk[word])
        return wti


    def _update_alphas(self, wt):
        for i in range(0, len(self._alphas)):
            self._alphas[i] = self._one_divided_by_N * wt[i]

    def _update_P(self, wt):
        for cluster in self._P:
            for word in cluster:
    ...
    def update_parameters(self):
        wt = list()
        for i in range(0, len(self._alphas)):
            wti = self._calculate_wti_numerator(i)
            wt.append(wti)
        wti_sums = sum(wt)
        for i in range(0, len(wt)):
            wt[i] /= wti_sums
        self._update_alphas(wt)
        self._update_P(wt)
